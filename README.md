...
- 修改标题、手动分类、删除书签；导出和恢复模型配置、自定义分类、书签增强数据与阅读快照。
- 中文界面、紧凑列表、系统深色模式。
...
API Key 存在 `chrome.storage.local`，访问级别设为可信扩展上下文，不传给内容脚本。完整备份会把 API Key 以明文写入 JSON 文件，以便在另一浏览器恢复模型配置；请只把备份保存在可信位置。它不是加密密码保险箱。使用 HTTP 的远程接口时，传输不加密，建议只给本机模型使用 HTTP。

阅读 HTML 在提取和显示时均经 DOMPurify 限制标签与属性；不执行保存页面的脚本。

卸载扩展会丢失增强数据和快照，原生 Chrome 书签仍然保留。请提前导出备份。备份中包含网址、正文和模型密钥，属于敏感个人数据。
